...
class FundLinkBuilder:
    ...
    def parse_update_logs_from_dart(self):
        """从 version_view.dart 解析 UPDATE_LOGS 列表"""
    ...
    def upload_to_nas(self, local_path, remote_subdir):
        ...
        try:
            with open(local_path, 'rb') as f:
                ssh_cmd = [
                    'ssh', '-p', str(NAS_CONFIG["port"]),
                    f"{NAS_CONFIG['username']}@{NAS_CONFIG['host']}",
                    f"cat > {remote_path}"
                ]
    ...
    def update_version_json(self):
        ...
        version_data = {
            ...
            "downloads": {
                "ios": {
                    "url": f"/downloads/ios/{ios_file}"
                },
                "android": {
                    "url": f"/downloads/android/{android_file}"
                },
                "windows": {
                    "url": "/downloads/windows/FundLink.zip"
                },
                "macos": {
                    "url": f"/downloads/macos/{macos_file}"
                }
            }
        }
    ...
    def build_ios(self):
        ...
        file_name = NAMING_CONFIG["pattern"].format(
            version=self.version, date=self.date_suffix
        ) + f".{NAMING_CONFIG['ios_ext']}"
    ...
    def build_android(self):
        ...
        build_args = ["flutter", "build", "apk", "--release",
                     "--target-platform", PROJECT_CONFIG["android_arch"]]
    ...
    def build_macos(self):
        ...
        possible_base_paths = [
            self.project_dir / "build/macos/Build/Products/Release",
            self.project_dir / "build/macosx/Build/Products/Release",
        ]
        ...
        if found_apps:
            for app in found_apps:
                if "FundLink" in app.name or "一基暴富" in app.name:
                    app_path = app
                    break
            if not app_path:
                app_path = found_apps[0]
            print(f"✅ 选择应用: {app_path.name}")
        else:
            print("🔍 使用 find 命令全面查找...")
            find_cmd = f'find {self.project_dir}/build -name "*.app" -type d 2>/dev/null'
            success, find_result = self.run_command(find_cmd, capture_output=True)
            if success and find_result:
                app_paths = find_result.strip().split('\n')
                print(f"找到 {len(app_paths)} 个 .app 文件:")
                for p in app_paths:
                    print(f"  - {p}")
                    if not app_path:
                        app_path = Path(p)
        ...
        if not app_path or not app_path.exists():
            ...
            print(f"❌ {error_msg}")
            print("请检查: ls -la build/macos/Build/Products/Release/")
        ...
        app_copy = temp_dir / f"{app_name}.app"
        print(f"复制应用: {app_path} -> {app_copy}")
        ...
        cmd_create = [
            "hdiutil", "create", "-volname", "FundLink",
        ...
        print("执行: hdiutil create...")
        success, create_output = self.run_command(cmd_create)
        if not success:
            print(f"❌ DMG 创建失败: {create_output}")
            shutil.rmtree(temp_dir)
...
def main():
    ...
    parser.add_argument('--only-platform', choices=['ios', 'android', 'macos', 'windows'], 
                       help='只打包指定平台')
