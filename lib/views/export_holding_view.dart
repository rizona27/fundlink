...
final rows = _filteredHoldings.map((h) {
...
context.showToast('导出成功，文件已开始下载');
} else {
...
case 'purchaseShares': return holding.purchaseShares.toStringAsFixed(2);
...
child: Row(
children: [
Icon(CupertinoIcons.exclamationmark_triangle_fill, size: 12, color: const Color(0xFFD46B6B)),
const SizedBox(width: 4),
Expanded(
child: Text(
_amountError,
style: TextStyle(fontSize: 11, color: const Color(0xFFD46B6B)),
),
),
],
),
...
const SizedBox(height: 12),
Text('导出成功！', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
const SizedBox(height: 8),
Text('文件名：$_exportedFileName', style: TextStyle(fontSize: 13, color: CupertinoColors.systemGrey)),
const SizedBox(height: 16),
GlassButton(
label: '完成',
onPressed: () => Navigator.of(context).pop(),
...
const SizedBox(height: 12),
Text('导出失败', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
const SizedBox(height: 8),
Text(_exportError!, style: TextStyle(fontSize: 13, color: CupertinoColors.systemGrey)),
const SizedBox(height: 16),
Row(
...
if (selected) Icon(CupertinoIcons.checkmark_alt, size: 18, color: const Color(0xFF8B9DC3)),
],
),
);
}
